...
export default function MLBookingWorkflow({ user, vehicles: initialVehicles, editingTrip, onClose, onConfirmed }) {
  ...
  const handleML = async () => {
    ...
    try {
      const res = await fetch('http://localhost:5007/api/ml/recommend', { method:'POST', headers:H, body:JSON.stringify({ ...prefs, Total_Days: totalDays }) });
  ...
  const handleDayPlan = async () => {
    ...
    setItinLoading(true);
    try {
      const res = await fetch('http://localhost:5007/api/ai/generate-itinerary', { method:'POST', headers:H, body:JSON.stringify({ selectedPlaces: allPlaces.map(p=>({ name:p.place, district:p.city, category:p.category })), duration:totalDays||1, startDate:dates.startDate }) });
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      let destinations = [];
      try {
        const destsRes = await fetch('http://localhost:5007/api/destinations');
      ...
      const url = editingTrip ? `http://localhost:5007/api/trips/${editingTrip.id}` : 'http://localhost:5007/api/trips';
